...
receiver:
如果收到的包超过window许可范围, 应只截取范围内的, 并且如果存在fin标志, 由于fin在逻辑上位于包的末尾, 需要优先去除它.

断断续续调试了3天, 通过了所有模拟测试, 聊天抓包也是正常的, 但真实测试(名字格式类似`t_ucS_1M_32K`)几乎全都无法通过......

真实测试的不同之处在于, 它是通过`sponge/txrx.sh`进行的, 测试名`t_ucS_1M_32K`意味着:
- u: 基于udp的tcp
- c: 测试中担任client
- S: 测试发送功能
- 1M: 总数据量为1MB
- 32K: 窗口宽度32KB
- 进行测试的命令为: `"${PROJECT_SOURCE_DIR}/txrx.sh" -ucSd 1M -w 32K`
...
有个很神奇的事情是, 测试结束的时候测试文件会被自动删除, 而且看不出来`txrx.sh`里面哪条指令是删除, 所以只好把测试文件复制到其它路径再进行查看.

可以自行进行小规模的测试, 1KB+32B窗口:
```
"${PROJECT_SOURCE_DIR}/txrx.sh" -ucSd 1K -w 32
```

这次测试发现只传了前32个字节, 更多测试表明少传字节数是共性的问题, 但不是每次都正好只传一个窗口的数据.

直觉上来说这是十分严重的问题, 但模拟测试测不出来, 这就很令人费解了. 

没什么思路, 继续阅读`txrx.sh`的核心逻辑.

在最底层的是`REF_PROG`和`TEST_PROG`这两个TCP实现, 顾名思义一个是用来参考的正确实现, 另一个是我们自己的实现. 

这两个TCP实现都可以发起监听或者发起连接, 因此我们有了2×2=4个函数:
- `ref_listen`
- `ref_connect`
- `test_listen`
- `test_connect`
...
在反复进行`128 -w 8`测试之后, 我意识到connection的回应发送不完全正确, sender对ackno和win_size的处理也有问题, 导致会出现无法关闭连接的情况, 所以重写了`fill_window()`和`ack_received()`.

进一步测试发现, ByteStream或者Reassembler也可能有问题, 所以先尝试把ByteStream从我手动实现的环形队列改成了deque, 现在终于能通过大部分真实测试了!! 但每次依然有6-10个左右的测试会Fail或者Timeout.

下一个发现的bug源于sender的计时重传机制, 似乎在outstanding(已发送但没被完全确认的段)为空的时候依然会重传, 导致segmentation fault. 我删掉了显式的私有变量`timer_running`, 改用outstanding是否为空来判断是否需要重传. 现在每次只剩1-3个测试会Timeout了. 

然后就发现可能大量调用`tick(0)`, 就有点离谱...不过特判一下就好.

在最后, 发现SYN_RECEIVED状态下可能收到AF, 这时候要直接切换到CLOSE_WAIT. 这种切换其实在wiki状态转换图上并不存在, 不过意识到就好办了. 
