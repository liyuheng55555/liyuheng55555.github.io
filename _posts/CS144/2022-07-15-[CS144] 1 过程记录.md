...
要求不是很全, 有些细节需要看测试才能知道, 测试封装得很到位, 很好读, 比如:
```c
...
实现对字节流的重组, 需要自行设计数据结构.

corner case非常多, 思路不对的话写出来的代码又长又乱还没法保证正确性, ~~我推倒重来了两次~~, 不过修改设计的过程挺有意思的.

最终使用的结构类似于`list<string>`, 并且即使遇到数据段连续的情况也不对string进行拼接, 因为拼接string耗时很长, 这样做虽然能减少`list`中元素数量但是得不偿失.
...
5. 可能有些部分插在`seg_list`最前面
6. 可能有些部分插在`seg_list`最后面
7. 向`seg_list`中间插入
8. 从头部开始扫描, 将组装好的部分移动到_output
9. 从尾部开始扫描, 去除超出`_capacity`的部分
10. 考虑是否`_output.end_input()`
